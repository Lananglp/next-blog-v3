                        ...
                        items.length > 0 ?
                            items.map((post, index) => {
                                return (
                                    <div
                                        key={index}
                                        className={`${hoveredId === post.id && selectedPosts.some((item) => item.id === post.id) ? 'bg-zinc-200/75 dark:bg-zinc-900/75' : 'hover:bg-zinc-200/75 dark:hover:bg-zinc-900/75'} px-3 py-3 border border-template rounded-lg flex items-center gap-3`}
